  ...
  return (
    <>
      <div className="lg:drawer bg-black lg:drawer-open hidden">
        ...
        <div className={`drawer-side shadow-lg fixed transition-all duration-1000 ${isCollapsed ? 'w-20' : 'w-64'}`}>
          <ul className="menu p-4 min-h-full gap-10 justify-center bg-black text-white ">
            <li>
              <Link href={'/dashboard'} className="flex items-end hover:text-gold gap-2">
                <IoHomeSharp className="text-lg text-white" />{' '}
                {!isCollapsed && <span>Dashboard</span>}
              </Link>
            </li>
            <li>
              <Link href={'/dashboard/events'} className="flex hover:text-gold items-end gap-2">
                <MdEventAvailable className="text-lg text-white" />{' '}
                {!isCollapsed && <span>My Events</span>}
              </Link>
            </li>
            <li>
              <Link href={'/dashboard/profile'} className="flex hover:text-gold items-end gap-2">
                <FaUserCircle className="text-lg text-white" />{' '}
                {!isCollapsed && <span>My Profile</span>}
              </Link>
            </li>
            <li>
              <button onClick={() => setIsCollapsed(!isCollapsed)} className="flex items-center hover:text-gold">
                {isCollapsed ? (
                  <FiChevronRight className="text-lg text-white" />
                ) : (
                  <FiChevronLeft className="text-lg text-white" />
                )}
                {!isCollapsed && <span className="ml-2 hover:text-gold">Collapse</span>}
              </button>
            </li>
          </ul>
        </div>
      ...
      <div className="flex flex-col min-h-screen lg:hidden">
        ...
        <div className="shadow-lg fixed bottom-0 left-0 w-full">
          <ul className="menu p-4 gap-10 bg-black text-white flex flex-row justify-around w-full">
            ...
            <li>
              <Link href={'/dashboard/events'} className='hover:text-gold'>
